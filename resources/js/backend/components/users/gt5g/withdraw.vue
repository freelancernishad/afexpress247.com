    ...
    methods: {
        ...
        async WithdrawonSubmit() {
            ...
            if(Number(this.form.amount)<Number(this.gateways.min_amount)) {
                ...
                if(Number(this.form.amount)>Number(this.row.user.balance)){
                    this.notifiy("You don't have sufficient balance");
                }else{
                    var id = localStorage.getItem('userid');
                    ...
                    if(res.data==422){
                        this.notifiy('A plan can be upgraded and then withdraw. thank you!');
